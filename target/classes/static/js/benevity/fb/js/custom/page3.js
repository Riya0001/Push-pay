...
function reviewAndCheckout() {
    const cart = sessionStorage.getItem(SS_CART_ITEM) || null;

    if (cart !== null && cart.length > 0) {
        if (SR !== '') {
            window.location.href = `/${SR}${STORE_ENDPOINT}/order/checkout/organization-details`;
        } else {
            window.location.href = `${STORE_ENDPOINT}/order/checkout/organization-details`;
        }
    } else {
        if (SR !== '') {
            window.location.href = `/${SR}${STORE_ENDPOINT}`;
        } else {
            window.location.href = STORE_ENDPOINT;
        }
    }
}
...
function customizeLink() {
    if (SR !== '') {
        window.location.href = `/${SR}${STORE_ENDPOINT}/order/customize`;
    } else {
        window.location.href = `${STORE_ENDPOINT}/order/customize`;
    }
}

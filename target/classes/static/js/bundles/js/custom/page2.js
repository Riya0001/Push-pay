...
const paymentOptionsByTerm = {
    "Monthly": ["Monthly"],
    "6 Months": ["Monthly (x6)", "Pay In Full"],
    "1 Year": ["Monthly (x12)", "Pay In Full"]
};

const defaultSubscriptionTerms = ["Monthly", "6 Months", "1 Year"];
const defaultTitleOptions = ["1 Device Bundle", "3 Devices Bundle"];
const deviceDenominationTexts = ["Choose your 1-device denomination setup", "Choose your 3-device denomination setup"]
...
function generateCards() {
    ...
    bundles.forEach((item, index) => {
        ...
        // Values
        const hideCardSecondarySection = bundles[index].title === "";
        ...
        const titleFormatted = isValid ? `${item.title !== '' ? item.title.toString() : 'bundle'}` :
            `Customize your ${item.title !== '' ? item.title.toString().toLowerCase() : 'bundle'}`;
                ...
                <div class="accordion-item">
                    ...
                    <div id="collapseDevice_${index}" class="accordion-collapse collapse ${isCollapse ? '' : 'show'}"
                        ...
                        <div class="accordion-body">
                            <div id="card-primary-section_${index}">
                                ${generateSelectYourBundleSection(index, item.title, defaultTitleOptions)}
                            </div>
                            
                            <div id="card-secondary-section_${index}" style="display: ${hideCardSecondarySection ? 'none' : 'block'}">
                                ${generateDeviceDenominationSection(index, item.title, item.deviceDenomination, template.denominationOptions)}
                                ${generateSolutionSelectionSection(index, item.solutionSelection, template.solutionOptions)}
...
// Function to generate the "Select Your Bundle" section
function generateSelectYourBundleSection(index, selectedBundleTitle, titleOptions) {
    let radioButtons = '';
    titleOptions.forEach((option, idx) => {
        radioButtons += `
            <div class="form-check me-4">
                <input class="form-check-input ${selectedBundleTitle === null || selectedBundleTitle === '' ? 'empty-radio' : ''}" type="radio" name="bundle-selection_${index}" id="bundle-selection-${index}-${idx}" ${selectedBundleTitle === option ? 'checked' : ''}>
                <label class="form-check-label ${selectedBundleTitle === null || selectedBundleTitle === '' ? 'empty-label' : ''}" for="bundle-selection-${index}-${idx}">${option}</label>
            </div>
        `;
    });
    return `
        <div class="mb-5">
            <div style="display: flex; align-items: center;">
                <span style="padding-right: 10px;">${svgContents.bundleSelection}</span>
                <span><span class="ttp-text-purple">Select Your Bundle</span></span>
            </div>
            ...
            <div class="mt-2 d-flex justify-content-start">
                ${radioButtons}
                 ...
                 <!-- Checkmark icon -->
                <div class="form-check me-4" id="checkmarkIcon-bundle-selection_${index}" 
                style="display: ${selectedBundleTitle === null || selectedBundleTitle === '' ? 'none' : 'inline-block'}">
                    <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="8" cy="8" r="6" stroke="#00C73C"/>
                        <path d="M5.33333 8L7.33333 10L10.6667 6" stroke="#00C73C"/>
                    </svg>
            ...
            <div id="empty-bundle-selection-text_${index}" class="empty-text-warnings" 
            style="display: ${selectedBundleTitle === null || selectedBundleTitle === '' ? 'block' : 'none'}">
                Please select a bundle type
            </div>
        </div>
    `;
}

// Function to generate the "Device Denomination" section
function generateDeviceDenominationSection(index, title, selectedDenomination, denominationOptions) {
    let radioButtons = '';
    denominationOptions.forEach((option, idx) => {
        radioButtons += `
            <div class="form-check me-4">
                <input class="form-check-input ${selectedDenomination === null || selectedDenomination === '' ? 'empty-radio' : ''}" type="radio" name="device-denomination_${index}" id="device-denomination-${index}-${idx}" ${selectedDenomination === option ? 'checked' : ''}>
                <label class="form-check-label ${selectedDenomination === null || selectedDenomination === '' ? 'empty-label' : ''}" for="device-denomination-${index}-${idx}">${option}</label>
            </div>
        `;
    });
    return `
        <div class="mb-5">
            <div style="display: flex; align-items: center;">
                ...
                <span><span class="ttp-text-purple">Device Denomination</span></span>
            </div>
            <div id="device-denomination-text_${index}" class="ttp-text-gray" style="padding-top: 10px; padding-bottom: 2px; font-size: 14px;">
                ${title === defaultTitleOptions[0] ? deviceDenominationTexts[0] : deviceDenominationTexts[1]}
            </div>
            <div id="radio-device-denomination-section_${index}" class="mt-2 d-flex justify-content-start">
                ${radioButtons}
                 <!-- Checkmark icon -->
                <div class="form-check me-4" id="checkmarkIcon-device-denomination_${index}" style="display: ${selectedDenomination === null || selectedDenomination === '' ? 'none' : 'inline-block'}">
                    <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="8" cy="8" r="6" stroke="#00C73C"/>
                        <path d="M5.33333 8L7.33333 10L10.6667 6" stroke="#00C73C"/>
                    </svg>
            ...
            <div id="empty-device-denomination-text_${index}" class="empty-text-warnings" style="display: ${selectedDenomination === null || selectedDenomination === '' ? 'block' : 'none'}">
...
// Function to generate the "Solution Selection" section
function generateSolutionSelectionSection(index, selectedSolution, solutionOptions) {
    let options = '';
    solutionOptions.forEach(option => {
        options += `<option value="${option}" ${selectedSolution === option ? 'selected' : ''}>${option}</option>`;
    });
    return `
        <div class="mb-5">
            <div style="display: flex; align-items: center;">
                <span style="padding-right: 10px;">${svgContents.solutionSelection}</span>
                <span>
                    <span class="ttp-text-purple">Display Selection</span>
                    <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" style="cursor: pointer" id="tooltipLikeModalTrigger_${index}">
                        <circle cx="8" cy="8.5" r="6" stroke="#222222"/>
                        <path d="M8.33334 5.50001C8.33334 5.6841 8.1841 5.83334 8.00001 5.83334C7.81591 5.83334 7.66667 5.6841 7.66667 5.50001C7.66667 5.31591 7.81591 5.16667 8.00001 5.16667C8.1841 5.16667 8.33334 5.31591 8.33334 5.50001Z" fill="#222222"/>
                        <path d="M8 11.8333V7.16667" stroke="#222222"/>
                    </svg>
                </span>
            </div>
            <div class="ttp-text-gray" style="padding-top: 10px; padding-bottom: 10px; font-size: 14px;">
                Choose your display type:
            </div>
            <div id="solution-selection-section_${index}" class="mt-2 d-flex justify-content-start">
                <select class="form-select custom-select-with-icon ${selectedSolution === null || selectedSolution === '' ? 'empty-field' : ''}" id="solution-selection_${index}">
                    <option hidden="" id="hidden-select-solution-option_${index}">Select Display</option>
                    ${options}
                </select>
                
                 <!-- Checkmark icon -->
                <span id="checkmarkIcon-solution-selection_${index}" class="check-icon" style="padding-top: 2.4rem; display: ${selectedSolution === null || selectedSolution === '' ? 'none' : 'inline-block'}">
                  <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="8" cy="8" r="6" stroke="#00C73C"/>
                    <path d="M5.33333 8L7.33333 10L10.6667 6" stroke="#00C73C"/>
                    </svg>
            ...
            <div id="empty-solution-selection-text_${index}" class="empty-text-warnings" style="display: ${selectedSolution === null || selectedSolution === '' ? 'block' : 'none'}">
                Please select a solution
            </div>
            <div class="form-check mb-3 mt-2">
                     <input class="form-check-input" type="checkbox" id="check-add-new-corps" name="option1" value="something">
                     <label class="ttp-text-gray"> Please check the box to learn about customized signage options. A sales associate will contact you after your order is placed to go over the details.</label>
                </div>
        </div>
    `;
}
...
// function addAccordionListener(index) {
//     const accordionButton = document.querySelector(`#accordionExampleDevice_${index} .accordion-button`);
//     const confirmDuplicateSection = document.getElementById(`custom-duplicate-label_${index}`);
//
//     if (accordionButton && confirmDuplicateSection) {
//         accordionButton.addEventListener('click', () => {
//             const validBundle = isBundleValidState(index);
//             if (validBundle) {
//                 const isExpanded = accordionButton.getAttribute('aria-expanded') === 'true';
//                 confirmDuplicateSection.style.display = isExpanded ? 'inline-flex' : 'none';
//             }
//         });
//     }
// }

function addBundleSelectionEventListener(index) {
    document.querySelectorAll(`input[name="bundle-selection_${index}"]`).forEach(radio => {
        radio.addEventListener('change', async function (event) {
            bundles[index].title = event.target.nextElementSibling.textContent;
            document.getElementById('bundle-card-title_' + index).textContent = "Customize your " + (event.target.nextElementSibling.textContent).toLowerCase();
            document.getElementById('device-denomination-text_' + index).textContent =
                bundles[index].title === defaultTitleOptions[0] ? deviceDenominationTexts[0] : deviceDenominationTexts[1];

            if (event.target.value === 'on') {
                // Reset Device Denomination
                bundles[index].deviceDenomination = '';
                document.getElementById('summary-bar-device-denomination_' + index).textContent = "--";
                // Reset Solution Selection
                bundles[index].solutionSelection = '';
                document.getElementById('summary-bar-solution-selection_' + index).textContent = "--";


                // Remove empty-radio
                document.querySelectorAll(`[id^="bundle-selection-${index}-"]`).forEach(function (element) {
                    element.classList.remove('empty-radio');
                });

                // Remove empty-label
                document.querySelectorAll(`[for^="bundle-selection-${index}-"]`).forEach(function (label) {
                    label.classList.remove('empty-label');
                });

                document.getElementById('empty-bundle-selection-text_' + index).style.display = 'none';

                showByID('checkmarkIcon-bundle-selection_' + index, true);

                checkIfRemoveEmptyCardBorderClass(index);

                renderRadioButtons(index,
                    bundles[index].title === oneDeviceBundleTemplate.title ?
                        oneDeviceBundleTemplate.denominationOptions : threeDeviceBundleTemplate.denominationOptions
                )

                renderSolutionSelection(index,
                    bundles[index].title === oneDeviceBundleTemplate.title ?
                        oneDeviceBundleTemplate.solutionOptions : threeDeviceBundleTemplate.solutionOptions
                )

                // Show card secondary section
                showByID('card-secondary-section_' + index, true);
            }

            // Update Summary Bar
            document.getElementById('summary-bar-bundle-selection_' + index).textContent = event.target.nextElementSibling.textContent;
...
function addDeviceDenominationEventListener(index) {
    document.querySelectorAll(`input[name="device-denomination_${index}"]`).forEach(radio => {
        radio.addEventListener('change', async function (event) {
            bundles[index].deviceDenomination = event.target.nextElementSibling.textContent;

            if (event.target.value === 'on') {
                // Remove empty-radio
                document.querySelectorAll(`[id^="device-denomination-${index}-"]`).forEach(function (element) {
                    element.classList.remove('empty-radio');
                });

                // Remove empty-label
                document.querySelectorAll(`[for^="device-denomination-${index}-"]`).forEach(function (label) {
                    label.classList.remove('empty-label');
                });

                document.getElementById('empty-device-denomination-text_' + index).style.display = 'none';

                showByID('checkmarkIcon-device-denomination_' + index, true);

                checkIfRemoveEmptyCardBorderClass(index);
            }

            // Update Summary Bar
            document.getElementById('summary-bar-device-denomination_' + index).textContent = event.target.nextElementSibling.textContent;
...
function addSolutionSelectionEventListener(index) {
    document.getElementById(`solution-selection_${index}`).addEventListener('change', async function (event) {
        bundles[index].solutionSelection = event.target.value;

        if (event.target.value !== null || event.target.value !== '' || event.target.value !== undefined) {
            document.getElementById('solution-selection_' + index).classList.remove('empty-field');

            document.getElementById('empty-solution-selection-text_' + index).style.display = 'none';

            showByID('checkmarkIcon-solution-selection_' + index, true);

            checkIfRemoveEmptyCardBorderClass(index);
        }

        // Update Summary Bar
        document.getElementById('summary-bar-solution-selection_' + index).textContent = event.target.value;
...
function addEventListeners(index) {
    ...
    addBundleSelectionEventListener(index);
    addDeviceDenominationEventListener(index);
    addSolutionSelectionEventListener(index);
    addSubscriptionTermEventListener(index);
    addPaymentOptionEventListener(index);
    addRemoveBundleEventListener(index);
    // addAccordionListener(index);
    addTooltipLikeModalEventListeners(index);
    addCustomizedSignageCheckboxEventListener(index);
    setEmptyOptionsTexts(index);
}
function addCustomizedSignageCheckboxEventListener(index) {
    const checkbox = document.getElementById(`check-add-new-corps`);
    if (checkbox) {
        checkbox.addEventListener('change', async function (event) {
            bundles[index].customizedSignage = event.target.checked;

            // Trigger price recalculation and cart UI update
            await setBundlesPriceKeyAndCalculate();
            generateSummaryBarDeviceTable();
            updateSummaryBarTotal();
        });
...
// function generateValidationResult(array) {
//     const container = document.getElementById("validation-results");
//     container.innerHTML = ""; // Clear container before rendering
//     const div = document.createElement('div');
//     div.id = `validation-result-list`;
...
// Function to render the radio buttons dynamically
function renderRadioButtons(index, currentDenominationArray) {
    const section = document.getElementById('radio-device-denomination-section_' + index);
    section.innerHTML = ''; // Clear the section

    currentDenominationArray.forEach((option, i) => {
        section.innerHTML += `
                <div class="form-check me-4">
                    <input class="form-check-input empty-radio" type="radio" name="device-denomination_${index}" id="device-denomination-${index}-${i}">
                    <label class="form-check-label empty-label" for="device-denomination-${index}-${i}">${option}</label>
                </div>
            `;
    });

    // Add the checkmark icon after the radio buttons
    section.innerHTML += `
            <div class="form-check me-4" id="checkmarkIcon-device-denomination_${index}" style="display: none">
                <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="8" cy="8" r="6" stroke="#00C73C"></circle>
                    <path d="M5.33333 8L7.33333 10L10.6667 6" stroke="#00C73C"></path>
                </svg>
            </div>
        `;

    showByID('empty-device-denomination-text_' + index, true);
    addDeviceDenominationEventListener(index);
}

// Function to render solution selections dynamically
function renderSolutionSelection(index, solutionOptionsArray) {
    const section = document.getElementById('solution-selection-section_' + index);
    section.innerHTML = ''; // Clear the section

    // Add the select element with dynamic options
    let selectHTML = `
        <select class="form-select custom-select-with-icon empty-field" id="solution-selection_${index}">
            <option hidden id="hidden-select-solution-option_${index}">Select Display</option>
    `;

    solutionOptionsArray.forEach(option => {
        selectHTML += `<option value="${option}">${option}</option>`;
    });

    selectHTML += `</select>`;

    // Add the checkmark icon after the select element
    const checkmarkHTML = `
        <span id="checkmarkIcon-solution-selection_${index}" class="check-icon" style="padding-top: 2.4rem; display: none">
            <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="8" cy="8" r="6" stroke="#00C73C"></circle>
                <path d="M5.33333 8L7.33333 10L10.6667 6" stroke="#00C73C"></path>
            </svg>
    ...
    section.innerHTML = selectHTML + checkmarkHTML;

    showByID('empty-solution-selection-text_' + index, true);
    addSolutionSelectionEventListener(index);
}
...
function addTooltipLikeModalEventListeners(index) {
    const tooltipLikeModalTrigger = document.getElementById('tooltipLikeModalTrigger_' + index);
    const tooltipLikeModal = document.getElementById('tooltipLikeModal');

    // Show the tooltip like modal on hover
    tooltipLikeModalTrigger.addEventListener('mouseenter', function () {
        const totalTextRect = tooltipLikeModalTrigger.getBoundingClientRect();

        // Set tooltip like modal position directly on top of the TOTAL text
        tooltipLikeModal.style.top = `${totalTextRect.top + window.scrollY - tooltipLikeModal.clientHeight - 190}px`;
        tooltipLikeModal.style.left = `${totalTextRect.left - 190}px`;

        // Display the tooltip modal
        tooltipLikeModal.classList.remove('visually-hidden');
    });

    // Hide the tooltip like modal when no longer hovering
    tooltipLikeModalTrigger.addEventListener('mouseleave', function () {
        tooltipLikeModal.classList.add('visually-hidden');
    });

    // Also hide the tooltip like modal if mouse leaves the custom modal itself
    tooltipLikeModal.addEventListener('mouseleave', function () {
        tooltipLikeModal.classList.add('visually-hidden');
    });
}

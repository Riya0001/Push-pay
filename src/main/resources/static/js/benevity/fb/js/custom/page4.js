...
function postData(formDataObj) {
    ...
    form.method = 'POST';
    if (SR !== '') {
        form.action = `/${SR}${STORE_ENDPOINT}/order/checkout/shipping`;
    } else {
        form.action = `${STORE_ENDPOINT}/order/checkout/shipping`;
    }
...
function redirectToBenevityOrder() {
    if (SR !== '') {
        window.location.href = `/${SR}${STORE_ENDPOINT}/order`;
    } else {
        window.location.href = `${STORE_ENDPOINT}/order`;
    }
}
